  ...
  methods: {
    ...
    addEdgeProperties(e) {
      ...
      this.EdgePropertyForm.validateFields((err, values) => {
        if (!err) {
          console.log('[addEdgeProperties] Received values of form: ', values);
          var test1 = /^[0-9][\s\S]*$/;
          ...
          console.log('[addEdgeProperties] all_property', this.all_property);
    ...
    addEdge(e) {
      ...
      this.edgeForm.validateFields(async (err, values) => {
        if (!err) {
          ...
          console.log(this.edgeData);
          await AddEdgeAPI({
            id: this.$store.getters.currentIndex,
            edge: this.edgeData,
          })
            .then((res) => {
              console.log(res);
