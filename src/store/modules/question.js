...
const question = {
  ...
  actions: {
    getAnswer: async ({ commit }, data) => {
      return await getAnswerAPI(data)
        .then((res) => {
          // console.log(res)
      ...
      await semanticSearchAPI(data)
        .then((res) => {
          ...
          for (var item in res.content.semanticStatement[0]) {
            ...
            if (
              context &&
              'polysemantNamedEntities' in context &&
              context.polysemantNamedEntities
            ) {
              if (context.polysemantNamedEntities.length != 0) {
                var imgPic = '';
                if (context.polysemantNamedEntities[0].picSrc) {
                  if (
                    context.polysemantNamedEntities[0].picSrc[0] == 'H' ||
                    context.polysemantNamedEntities[0].picSrc[0] == 'h'
                  ) {
                ...
                list_of_value.push({
                  name: context.name,
                  url: context.polysemantNamedEntities[0].entityUrl
                    ? context.polysemantNamedEntities[0].entityUrl
                    : '',
                  pic:
                    imgPic != ''
                      ? context.polysemantNamedEntities[0].picSrc
    ...
    getRecommand: async ({ commit }, data) => {
      return await getRecommandAPI(data)
        .then((res) => {
